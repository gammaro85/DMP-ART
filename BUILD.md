...
## End User Quick Start (Portable Package)

After unpacking `DMP-ART-Portable.zip`:

1. Double-click `start_portable.bat`
2. Browser opens automatically at `http://localhost:5000`
3. Upload a PDF/DOCX NCN proposal and work as usual

This package avoids a custom PyInstaller-generated `.exe`.
It starts the app through a bundled Python runtime and keeps all files local in the unpacked folder.

**Folder layout inside the ZIP:**
```
DMP-ART-Portable/
├── start_portable.bat     ← preferred startup for non-technical Windows users
├── start_portable.ps1     ← PowerShell alternative
...
│   ├── reviews/
│   └── cache/
├── config/                ← editable comments and settings
└── URUCHOM-NAJPIERW.txt   ← short end-user instructions
```
...
## 📦 Portable Runtime Package (Recommended for End Users on Windows)

### Quick Build

```bash
# Build portable package with copied local Python runtime
python build_portable.py
```

**Output:** `DMP-ART-Portable.zip`

### Why this variant

- no custom `DMP-ART.exe` generated by PyInstaller
- simpler trust story for Windows Defender and institutional endpoint protection
- same local-only storage model for uploads, cache, output files, and comment configuration
- one-click startup via `start_portable.bat`

### Notes

- The portable package is platform-specific. Build it on the same OS family you want to distribute.
- The current implementation targets Windows packaging because that is where the `.exe` blocking problem appears.
- OCR for scanned PDFs still requires a separate Tesseract installation.

## 📦 Standalone Executable (Legacy Option)
